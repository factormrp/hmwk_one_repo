========= MASTER -OLD =========

1st commit: inclusion of non-templated vector class provided by professor

2nd commit: isolation of vector header file and companion driver

3rd commit: fixed linkage of header to drivers

4th commit: modified drivers by adding parentheses to ensure order of operations and also added messages for big 4 functions

5th commit: merged into template

=========== TEMPLATE ==========

4th commit: created new branch to develop templated vector class

5th commit: defined template class and overloads

6th commit: fixed templating compilation errors

7th commit: merged master into branch and added gitignore

========= MASTER -NEW =========

8th commit: deleted unneccessay driver that came with initial vector class

9th commit: renamed template as master and updated README